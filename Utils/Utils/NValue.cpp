...
NValue::NValue(LPCSTR value)
{
	...
	std::wstring wstr = Utils::AnsiToUnicode(value);
	_data.strVal = wstr;
}
...
{
	std::wstring wstrRight = Utils::AnsiToUnicode(right);
...
bool NValue::operator !=(LPCSTR right) const
{
	std::wstring wstr = Utils::AnsiToUnicode(right);
...
{
	ToString();
	std::string str = Utils::UnicodeToAnsi(_data.strVal);
	return str;
}
