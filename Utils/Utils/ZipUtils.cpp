	...
	namespace ZipUtils {
		...
		bool zipFolder(HZIP& hz, const std::wstring& folder, const std::wstring& basePath)
		{
			...
			while (FindNextFile(hFile, &fData) != 0)
			{
				...
				{
					std::wstring subFolder = Utils::FileUtils::FullPath(folder, fData.cFileName);
				...
				{
					std::wstring fileName = Utils::FileUtils::FullPath(folder, fData.cFileName);
		...
		bool ZipDirectory(const std::wstring& directory, const std::wstring& zipName)
		{
			...
			dir = Utils::FileUtils::TrimSlash(dir);
			std::wstring srcName, srcPath;
			Utils::FileUtils::SplitPath(dir, srcPath, srcName);

			std::wstring dstName = zipName;
			if (zipName.empty())
			{
				dstName = Utils::FileUtils::FullPath(srcPath, srcName);
		...
		bool UnzipFile(const std::wstring& zipName, const std::wstring& path)
		{
			...
			if (path.empty())
			{
				...
				Utils::FileUtils::SplitPathEx(zipName, srcPath, driver, srcName, srcExt);
		...
		bool ZipFile(const std::wstring& fileName, const std::wstring& zipName)
		{
			...
			Utils::FileUtils::SplitPath(fileName, srcDir, srcName);
			...
			else
			{
				if (zipName.empty())
				{
					std::wstring nameNoExt = Utils::FileUtils::FileName(fileName, false);
					fullZipName = nameNoExt + L".zip";
				}

				fullZipName = Utils::FileUtils::FullPath(srcDir, fullZipName);
			}
			
			
			std::wstring dstPath, dstName;
			Utils::FileUtils::SplitPath(fullZipName, dstPath, dstName);
